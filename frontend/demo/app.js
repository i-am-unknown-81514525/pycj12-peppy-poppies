...
const handleLogin = async () => {
    ...
    const jwtToken = getCookie('CODECAPTCHA_JWT');
    if (!jwtToken) {
        showStatus('JWT token not found. Please complete the captcha challenge again.', 'error');
        appState.captchaCompleted = false;
        updateCaptchaButton();
        return;
    }
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            appState.jwtPayload = result.jwt_payload;
            ...
            if (loginBtn) {
                ...
                loginBtn.innerHTML = '<i class="fas fa-sign-in-alt"></i> Sign In to Account';
                loginBtn.className = 'btn btn-primary';
            }
        }
    } catch (error) {
        console.error('Login error:', error);
        showStatus('Login failed: Network error. Please try again.', 'error');
        if (loginBtn) {
            loginBtn.disabled = false;
            loginBtn.innerHTML = '<i class="fas fa-sign-in-alt"></i> Sign In to Account';
            loginBtn.className = 'btn btn-primary';
